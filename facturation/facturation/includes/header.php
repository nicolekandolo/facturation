...
/**
 * En-tête - incluable dans les pages
 */
if (!defined('CONFIG_LOADED')) {
    ...
    require_once __DIR__ . '/fonctions-factures.php';
...
<body class="bg-slate-950 text-slate-200 min-h-screen flex flex-col">
    ...
    <header class="bg-slate-900 border-b border-slate-800 px-5 py-3 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="/facturation/auth/logout.php" title="Déconnexion"
    ...
    <div class="flex flex-1 flex-col lg:flex-row">
        ...
        <nav class="bg-slate-900/60 lg:w-60 border-b lg:border-b-0 lg:border-r border-slate-800 p-3 flex lg:flex-col gap-2 overflow-x-auto lg:overflow-x-visible">
            <?php
            $nav_items = [
                ['index.php',                      'Facturation / POS',   'caissier|manager|super_admin', 'M3 3h2l.4 2M7 13h10l4-4H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z'],
                ['modules/produits/liste.php',     'Produits / Stocks',   'manager|super_admin', 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4'],
                ['modules/facturation/afficher-facture.php', 'Factures / Rapports', 'manager|super_admin', 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z'],
                ['admin/gestion-comptes.php',      'Gérer Comptes',       'super_admin', 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z'],
            ];

            foreach ($nav_items as [$path, $label, $roles_str, $icon]):
                $roles = explode('|', $roles_str);
                if (!in_array($user['role'], $roles)) continue;

                $page = basename($_SERVER['PHP_SELF']);
                $active = strpos($path, $page) !== false || ($page === 'index.php' && $path === 'index.php');
            ?>
                <a href="<?= $path ?>"
